...
do
	...
	function a.e()
		...
		return function(e)
			...
			function f.Lock(k)
				d(f.UIElements.Main.Lock, 0.08, {
					BackgroundTransparency = 0.6
				}):Play()
				d(f.UIElements.Main.Lock.ImageLabel, 0.08, {
					ImageTransparency = 0
				}):Play()
		...
		function e.New(f, g)
			...
			k.MouseButton1Click:Connect(function()
				if i then
					...
					g.WindUI:Notify{
						Title = '复制成功!',
						Content = "文件'" .. h.Title .. "' 已经成功复制到您的粘贴板里",
						Icon = 'clipboard-copy',
						Duration = 5
					}
				end
	...
	function a.o()
		...
		function n.Colorpicker(o, p, q)
			...
			end, CreateInput('十六进制', '#' .. r.Default:ToHex())
			local J, K, L, M = CreateInput('红', H(r.Default).R), CreateInput('绿', H(r.Default).G), (CreateInput('蓝', H(r.Default).B))
	...
	function a.q()
		...
		function d.SelectTab(e, f)
			...
			for g, h in next, d.Tabs do
				c(h.UIElements.Main.TextLabel, 0.15, {
					TextTransparency = 0.4
				}):Play()
				if h.Icon and ab.Icon(h.Icon) then
					c(h.UIElements.Main.ImageLabel, 0.15, {
						ImageTransparency = 0.4
					}):Play()
				end
				h.Selected = false
			end
			c(d.Tabs[f].UIElements.Main.TextLabel, 0.15, {
				TextTransparency = 0
			}):Play()
			if d.Tabs[f].Icon and ab.Icon(d.Tabs[f].Icon) then
				c(d.Tabs[f].UIElements.Main.ImageLabel, 0.15, {
					ImageTransparency = 0
				}):Play()
			end
	...
	function a.r()
		...
		return function(f)
			...
			function g.Open(G)
				...
				d(g.UIElements.Main.Main, 0.25, {
					GroupTransparency = 0
				}, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out):Play()
				d(g.UIElements.Main.UIScale, 0.25, {
					Scale = 1
				}, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out):Play()
				d(s, 0.25, {
					BackgroundTransparency = 0
			...
			w.MouseButton1Click:Connect(function()
				...
				local K = x and 'Press ' .. g.ToggleKey.Name .. ' 打开ui' or '单击“打开”来重新显示ui'
				if not e then
					e = not e
					f.WindUI:Notify{
						Title = '最小化',
						Content = "你把ui最小化了，可通过“打开”旁的图像进行拖动" .. K,
						Icon = 'eye-off',
						Duration = 5
					}
				end
...
function aa.CreateWindow(j, k)
	...
	function KeySystem()
		...
		if k.KeySystem.SaveKey and k.Folder then
			d('TextLabel', {
				Text = '剑客不得用于任何恶意行为，包括但不限于破坏游戏公平性。',
				--'你的卡密将会自动保存到本地Workspace/' .. k.Folder .. '/' .. q .. '.key',
				TextSize = 13,
				FontFace = Font.new(c.Font, Enum.FontWeight.SemiBold),
				TextXAlignment = 'Left',
				TextWrapped = false,
